% Plots the average z-scored MAD of each subject.
% plt_p - struct of plotting params.
% p - struct of exp params.
function [p_val] = plotMultiMad_z(traj_names, plt_p, p)
    for iTraj = 1:length(traj_names)
        ...
        beesdata = {reach_avg_each(iTraj).mad_z.con_left(good_subs), reach_avg_each(iTraj).mad_z.incon_left(good_subs), reach_avg_each(iTraj).mad_z.con_right(good_subs), reach_avg_each(iTraj).mad_z.incon_right(good_subs)};
        yLabel = 'Z-scored MAD (meter)';
        XTickLabels = [];
        ...
        % T-test On plot
        [~, p_val_mad_z, ci, ~] = ttest(beesdata{1}, beesdata{2});
        text(mean(ticks(1:2)), (max([beesdata{1:2}])+0.005), ['p: ' num2str(p_val_mad_z)], 'HorizontalAlignment','center', 'FontSize',14);
        [~, p_val_mad_z, ci, ~] = ttest(beesdata{3}, beesdata{4});
        text(mean(ticks(3:4)), (max([beesdata{3:4}])+0.005), ['p: ' num2str(p_val_mad_z)], 'HorizontalAlignment','center', 'FontSize',14);
        % T-test and Cohen's dz
        [~, p_val, ci, stats] = ttest(reach_avg_each.mad_z(iTraj).con(good_subs), reach_avg_each.mad_z(iTraj).incon(good_subs));
        printStats('-----Z-scored MAD------------', reach_avg_each.mad_z(iTraj).con(good_subs), ...
            reach_avg_each.mad_z(iTraj).incon(good_subs), ["Con","Incon"], p_val, ci, stats);
    end
end